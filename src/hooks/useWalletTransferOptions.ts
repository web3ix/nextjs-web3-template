...
export default function useWalletTransferOptions() {
	...
	const { addContractWallet, getContractWallet, updateContractWallet } =
		useContractWalletsStore();
	...
	useEffect(() => {
		...
		if (!contractWallet) {
			...
			const sourceNetworkFromSettings = networks.find(
				(n) => n.name === source_network.name
			);
			checkContractWallet(wallet.address, sourceNetworkFromSettings).then(
				(result) => {
					updateContractWallet(
						wallet.address,
						sourceNetworkFromSettings?.name,
						result
					);
				}
			);
	...
	const walletAddressType = getContractWallet(
		wallet?.address,
		source_network?.name
	);

	const canDoSweepless =
		(source_network &&
			((source_network.type == NetworkType.EVM &&
				walletAddressType?.ready &&
				!walletAddressType?.isContract) ||
				source_network.type == NetworkType.Starknet ||
				source_network.type == NetworkType.ZkSyncLite)) ||
