...
export default function useWallet() {
	...
	const getWithdrawalProvider = (network: Network) => {
		const provider = WalletProviders.find((provider) =>
			provider.withdrawalSupportedNetworks.includes(network.name)
		);
		return provider;
	};
	...
	const getAutofillProvider = (network: Network) => {
		const provider = WalletProviders.find((provider) =>
			provider?.autofillSupportedNetworks?.includes(network.name)
		);
		return provider;
	};
	...
	const getSourceProvider = (network: Network) => {
		const provider = WalletProviders.find((provider) =>
			provider?.asSourceSupportedNetworks?.includes(network.name)
		);
		return provider;
	};
