	...
	public static Initialize() {
		...
		for (var k in NetworkSettings.KnownSettings) {
			...
			if (networkSetting) {
				...
				networkSetting.OrderInDestination =
					destOrder < 0 ? destinationOrder.length : destOrder;
				networkSetting.OrderInSource =
					srcOrder < 0 ? destinationOrder.length : srcOrder;
			}
		}
	}
}
