...
export default function useStarknetBalance(): BalanceProvider {
	...
	const getNetworkBalances = async ({
		...
		const { Contract, RpcProvider, uint256 } = await import("starknet");
		...
		for (let i = 0; i < network.tokens.length; i++) {
			try {
				...
				const balanceInWei = BigNumber.from(
	...
	const getGas = async ({ network, token, wallet }: GasProps) => {
		...
		const testnetWatchdog =
			"0x0423074c4bf903478daaa719bb3b1539d23af07db07101d263c78d75e5e6e0a3";
		const mainnetWatchdog =
		...
		const version =
			network.name.split("_").pop() === "SEPOLIA" ||
			network.name.split("_").pop() === "GOERLI"
				? "sandbox"
				: "prod";

		const recipient = version === "prod" ? mainnetRecipient : testnetRecipient;
		const watchdogContract =
			version === "prod" ? mainnetWatchdog : testnetWatchdog;

		if (!token || !network.token) return;

		const client = new InternalApiClient();
		const basePath = router.basePath ?? "/";
		const feeEstimateResponse: ApiResponse<EstimateFee> =
			await client.GetStarknetFee(
				`nodeUrl=${nodeUrl}&walletAddress=${wallet?.address}&contractAddress=${contract_address}&recipient=${recipient}&watchDogContract=${watchdogContract}`,
				basePath
			);

		if (!feeEstimateResponse?.data?.suggestedMaxFee) {
			throw new Error(
				`Couldn't get fee estimation for the transfer. Response: ${JSON.stringify(
					feeEstimateResponse
				)}`
			);
		}
