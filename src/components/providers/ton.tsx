...
const TonConnectProvider = ({
	...
	const rgbToHex = (rgb: string) => {
		...
		return (
			"#" +
			componentToHex(Number(rgbArray[0])) +
			componentToHex(Number(rgbArray[1])) +
			componentToHex(Number(rgbArray[2]))
		);
	};
	...
	return (
		<TonConnectUIProvider
			...
			manifestUrl={`https://${process.env.NEXT_PUBLIC_VERCEL_URL}${
				basePath ? `/${basePath}` : ""
			}/tonconnect-manifest.json`}
			actionsConfiguration={{
				twaReturnUrl:
					appName === "4233c46e96e44017afae91537841cb46"
						? "https://t.me/layerswap_bridge_bot/bridge"
						: undefined,
			}}
		>
			{children}
